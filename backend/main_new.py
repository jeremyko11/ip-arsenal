# ─── UTF-8 编码（由 config.py 处理，但保留用于非 import 场景）────────────
import sys, os
...
# ─── 核心模块导入 ─────────────────────────────────────────────────────
from config import (
    DB_PATH, DATA_DIR, UPLOAD_DIR, FRONT_DIR,
...
# ─── Worker 队列（已迁移到 tasks.py，此处保留引用兼容）──────────────
...
# ⚠️ 以下为占位替换，原有内联代码已迁移至独立模块
# config.py    - 常量、AI客户端
# db.py        - 数据库初始化
# ai.py        - AI调用链、超时处理
# tasks.py     - Worker线程池
# extraction.py - PDF/EPUB/TXT文本提取
# prompts.py   - 提示词构建、素材解析
